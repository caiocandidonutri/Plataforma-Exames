  ...
  try {
    ...
    const safeFetch = async (url: string, options?: RequestInit) => {
      try {
        ...
        throw new Error(
          `Falha de conexão com a API (${url}). A URL pode estar incorreta, inacessível (DNS/Name or service not known) ou a conexão foi recusada.`,
        )
      }
    }
    ...
    const processGatewayResponse = async (res: Response) => {
      ...
      if (!res.ok) {
        if (res.status === 400 || res.status === 404) {
          throw new Error(
            'A rota da API não foi encontrada ou os parâmetros são inválidos. Verifique as configurações.',
          )
        }
    ...
    const getApiConfig = async (type: 'whatsapp' | 'hostinger' = 'whatsapp') => {
      ...
      if (
        !apiUrl ||
        apiUrl.includes('seudominio.com') ||
        apiUrl.includes('sua-api-whatsapp.com') ||
        !apiToken ||
        apiToken.includes('seu-token')
      ) {
        console.error('Gateway: API validation failed - missing or placeholder credentials.')
        throw new Error(
          "A URL da API ainda está configurada com um valor de exemplo. Por favor, acesse 'Configurar Infraestrutura' e insira a URL correta da sua Evolution API.",
        )
      }
      ...
      try {
        ...
        throw new Error(
          `A URL configurada (${apiUrl}) não é válida. Verifique se começa com http:// ou https://`,
        )
      }
    ...
    switch (action) {
      case 'testConnection': {
        ...
        try {
          ...
          if (res.status === 401 || res.status === 403) {
            return new Response(
              JSON.stringify({
                error: 'Credenciais inválidas. Verifique a Evolution API Global Key.',
              }),
          ...
          if (!res.ok) {
            if (res.status === 404 || res.status === 400) {
              return new Response(
                JSON.stringify({
                  error:
                    'Erro de Rota: Verifique se a URL da Evolution API está correta e se o Token é a Global API Key.',
                }),
                { status: 400, headers: { ...corsHeaders, 'Content-Type': 'application/json' } },
              )
            }
            return new Response(
              JSON.stringify({ error: `Erro no servidor da API: ${res.status}` }),
              { status: 400, headers: { ...corsHeaders, 'Content-Type': 'application/json' } },
            )
          }
          return new Response(
          ...
          return new Response(JSON.stringify({ error: `Servidor inacessível: ${e.message}` }), {
            status: 400,
            headers: { ...corsHeaders, 'Content-Type': 'application/json' },
          })
        }
      ...
      case 'testHostingerConnection': {
        ...
        try {
          ...
          return new Response(
            JSON.stringify({ error: `URL inválida para Hostinger: ${testUrl}` }),
            { status: 400, headers: { ...corsHeaders, 'Content-Type': 'application/json' } },
          )
        }
        ...
        try {
          ...
          if (res.status === 401 || res.status === 403) {
            return new Response(
              JSON.stringify({ error: 'Token da Hostinger inválido ou sem permissão.' }),
              { status: 400, headers: { ...corsHeaders, 'Content-Type': 'application/json' } },
            )
          }
          if (!res.ok) {
          ...
          return new Response(
            JSON.stringify({ success: true, message: 'Conexão estabelecida com sucesso!', data }),
            { headers: { ...corsHeaders, 'Content-Type': 'application/json' } },
          )
        } catch (e: any) {
          return new Response(
            JSON.stringify({ error: `Servidor Hostinger inacessível: ${e.message}` }),
            { status: 400, headers: { ...corsHeaders, 'Content-Type': 'application/json' } },
          )
        }
      }
      ...
      case 'createInstance': {
        ...
        const safeInstanceName =
          payload.instance_name.replace(/[^a-zA-Z0-9]/g, '') +
          '-' +
      ...
      case 'getQrCode': {
        ...
        if (!qrBase64) {
          ...
          throw new Error(
            'Não foi possível gerar o QR Code. A instância pode já estar conectada ou em estado inválido.',
          )
        }
        ...
        if (error) throw error
        return new Response(JSON.stringify({ success: true, qr_code_base64: qrBase64 }), {
          headers: { ...corsHeaders, 'Content-Type': 'application/json' },
        })
      }
      ...
      case 'checkStatus': {
        ...
        try {
          ...
          return new Response(
            JSON.stringify({ status: instance.status, phone_number: instance.phone_number }),
            { headers: { ...corsHeaders, 'Content-Type': 'application/json' } },
          )
        }
        ...
        try {
          ...
          return new Response(JSON.stringify({ status: newStatus, phone_number: phone }), {
            headers: { ...corsHeaders, 'Content-Type': 'application/json' },
          })
        } catch (e) {
          return new Response(
