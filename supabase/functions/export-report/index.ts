...
Deno.serve(async (req: Request) => {
  ...
  try {
    ...
    const { data: clinica } = await supabase
      ...
      .limit(1)
      .maybeSingle()
    const clinicaNome = clinica?.nome_clinica || 'Sua Clínica'
    const clinicaCnpj = clinica?.cnpj ? ` | CNPJ: ${clinica.cnpj}` : ''
    const clinicaRegistro = clinica?.registro_profissional
      ? ` | Registro: ${clinica.registro_profissional}`
      : ''

    const header = `Clínica: ${clinicaNome}${clinicaCnpj}${clinicaRegistro}\n`

    const fileContent = `${header}Relatorio: ${tipoRelatorio}\nFormato: ${formato}\nGerado em: ${new Date().toISOString()}\n\n-- DADOS --\nRegistro 1; Valor 1\nRegistro 2; Valor 2\n\n[LGPD] Este documento contém dados sensíveis. Uso restrito e confidencial.`
