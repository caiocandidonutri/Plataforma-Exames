...
Deno.serve(async (req: Request) => {
  ...
  try {
    ...
    const { data: exportacao, error: createError } = await supabase.from('exportacao_relatorio').insert({
      usuario_id: user.id,
    ...
    const contentType = formato === 'csv' ? 'text/csv' : formato === 'pdf' ? 'application/pdf' : 'application/vnd.openxmlformats-officedocument.spreadsheetml.sheet'
    ...
    await supabase.from('exportacao_relatorio').update({
      status: 'pronto',
    ...
    return new Response(JSON.stringify({ success: true, id: exportacao.id, url: signedData.signedUrl }), { headers: { ...corsHeaders, 'Content-Type': 'application/json' } })
